...
public class LabCreator implements Creator {
    ...
    public LabCreator(GameScreen screen)
    {
        ...
        allBodies.addAll(PKMUtils.createBody(screen, PKMConstants.LAB_ENVIRONMENT));

        //exits
        exits = new Array<>();
        exits.addAll(PKMUtils.createDoors(screen, PKMConstants.LAB_EXIT, exits));
        for(Door exit : exits) {
    ...
    public void draw(Pokemon game, GameScreen gameScreen) {
        for(Door door : exits) {
            ...
            if (door.isInteracted()) {
                if (door.getProperties(PKMConstants.PROPERTY_LAB)) {
